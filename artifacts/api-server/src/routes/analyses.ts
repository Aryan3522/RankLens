...
import { db, analysesTable, seoIssuesTable, recommendationsTable } from "@workspace/db";
import {
  CreateAnalysisBody,
  GetAnalysisParams,
...
import { generateSeoAnalysis } from "../lib/seo-analyzer";
...
router.get("/analyses", async (req, res): Promise<void> => {
  ...
  const analyses = conditions.length > 0
    ? await db.select().from(analysesTable).where(and(...conditions)).orderBy(analysesTable.createdAt)
    : await db.select().from(analysesTable).orderBy(analysesTable.createdAt);
...
router.post("/analyses/:id/rerun", async (req, res): Promise<void> => {
  ...
  const [existing] = await db.select().from(analysesTable).where(eq(analysesTable.id, params.data.id));
  if (!existing) {
    res.status(404).json({ error: "Analysis not found" });
    return;
  }
